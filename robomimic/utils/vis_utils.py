...
def write_text_on_image(image_arr, text, def_color=None, font=30, pos=(15, 15)):
    img = Image.fromarray(image_arr)
    image_edit = ImageDraw.Draw(img)
    font = ImageFont.truetype("FreeMono.ttf", font)
    color = (0, 255, 0) if def_color is None else def_color
    image_edit.text(pos, text, color, font=font)
    return np.array(img)
