...
class ConfigGenerator(object):
    ...
    def __init__(self, base_config_file, wandb_proj_name='debug', script_file=None, generated_config_dir=None):
        ...
        self.wandb_proj_name = wandb_proj_name.lower()

    def add_param(self, key, name, group, values=None, value_names=None, values_and_names=None, hidename=False):
        """
    ...
    def _generate_jsons(self):
        ...
        # # prepend date to experiment
        # time_str = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d-')
        # base_exp_name = time_str + base_exp_name
        ...
        for i in range(num_settings):
            ...
            json_dict['tags'] = dict(
                wandb_proj_name=self.wandb_proj_name,
            ...
            t_now = time.time()
            time_str = datetime.datetime.fromtimestamp(t_now).strftime('%m-%d-%H-%M-%S')
            json_path = os.path.join(base_dir, "{}-{}.json".format(time_str, exp_name))
